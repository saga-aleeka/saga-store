...
app.post('/make-server-aaac77aa/samples', async (c: any) => {
  try {
    ...
    // Persist sample movement events and update container location on movement
    app.post('/make-server-aaac77aa/sample-movements', async (c: any) => {
      try {
        const body = await c.req.json();
        const {
          timestamp,
        ...
        if (!sample_id || !action_type) {
          return c.json({ error: 'sample_id and action_type required' }, 400);
        }

        // Idempotency: if client provided an idempotency_key and an entry already exists, return success
        const idempotencyKey = body.idempotency_key || body.idempotencyKey || null;
        if (idempotencyKey) {
          const { data: existing, error: exErr } = await supabase.from('sample_movements').select('id').eq('idempotency_key', idempotencyKey).limit(1);
          if (exErr) {
            console.error('Error checking idempotency key:', exErr);
        ...
        // Insert movement event into table
        const insertPayload: any = {
          ...
          sample_id,
          action_type,
          from_container_id: from_container_id || null,
          from_position: from_position || null,
          to_container_id: to_container_id || null,
          to_position: to_position || null,
          notes: notes || null,
          success: success !== false
        };

        const { error: insertErr } = await supabase.from('sample_movements').insert([insertPayload]);
        if (insertErr) throw insertErr;

        // If movement included a to_container_id and to_position, update samples table and container location mapping
        try {
          if (to_container_id && to_position) {
            // Attempt to upsert the sample row to the new container and position
            // First, find existing sample by sample_id
            const { data: existingSamples, error: fetchErr } = await supabase.from('samples').select('*').eq('sample_id', sample_id);
            if (fetchErr) throw fetchErr;
            if (Array.isArray(existingSamples) && existingSamples.length > 0) {
              // Update first match to move it
              ...
              const { error: upErr } = await supabase.from('samples').update(updatePayload).eq('id', sampleRow.id);
              if (upErr) throw upErr;
            } else {
              // Insert a new sample record if none exists
              const insertSample = {
                sample_id,
                container_id: to_container_id,
                position: to_position,
              ...
              const { error: insErr } = await supabase.from('samples').insert([insertSample]);
              if (insErr) throw insErr;
            }

            // Also, update container occupied_slots counts for from/to containers
            try {
              if (from_container_id) {
                const { data: fromSamples } = await supabase.from('samples').select('id').eq('container_id', from_container_id);
                const fromCount = Array.isArray(fromSamples) ? fromSamples.length : 0;
                await supabase.from('containers').update({ occupied_slots: fromCount }).eq('id', from_container_id);
              }
              if (to_container_id) {
                const { data: toSamples } = await supabase.from('samples').select('id').eq('container_id', to_container_id);
                const toCount = Array.isArray(toSamples) ? toSamples.length : 0;
                await supabase.from('containers').update({ occupied_slots: toCount }).eq('id', to_container_id);
              }
            } catch (cntErr) {
              console.error('Failed to update container counts after movement:', cntErr);
            }
          }
        } catch (movementErr) {
          console.error('Error applying movement to samples/containers:', movementErr);
        }

        // Create a server-side audit log entry
        await createAuditLogEntry('sample_moved', 'sample', sample_id, user_id || 'system', {
          description: `Sample movement: ${sample_id} -> ${to_container_id || 'N/A'} @ ${to_position || 'N/A'}`,
          metadata: { from_container_id, from_position, to_container_id, to_position }
        });

        app.get('/make-server-aaac77aa/sample-movements', async (c: any) => {
          try {
            const limit = parseInt(c.req.query('limit') || '200');
            // Try DB first
            const { data, error } = await supabase.from('sample_movements').select('*').order('timestamp', { ascending: false }).limit(limit);
            if (!error && Array.isArray(data)) {
              return c.json({ movements: data });
            }

            // Fallback: return movements from KV (if any)
            const kvMovements = await kv.get('saga-sample-movements') || [];
            return c.json({ movements: Array.isArray(kvMovements) ? kvMovements.slice(0, limit) : [] });
          } catch (err) {
            console.error('Error fetching sample movements:', err);
        ...
        console.error('Server error inserting sample movement:', err);
        const errorMessage = err instanceof Error ? err.message : 'Unknown error';
        return c.json({ error: 'Internal server error', details: errorMessage }, 500);
      }
    });
