...
// Persist a sample movement event via server functions (graceful fallback to localStorage)
export async function recordMovement(event: {
  timestamp?: string;
  userId?: string;
  userName?: string;
  sampleId: string;
  ...
  try {
    const payload = {
      ...
      from_container_id: event.fromContainerId,
      from_position: event.fromPosition,
      to_container_id: event.toContainerId,
      to_position: event.toPosition,
      notes: event.notes,
      success: event.success !== false
    };

    // Add an idempotency key so retries don't create duplicate movement records
    const idempotencyKey = `move-${event.sampleId}-${Date.now()}-${Math.random().toString(36).substr(2,8)}`;
    // Attach to payload
    (payload as any).idempotency_key = idempotencyKey;

    const response = await fetchWithTimeout(`${API_BASE_URL}/sample-movements`, {
      method: 'POST',
    ...
    console.warn('Failed to record movement to server, storing locally:', err);
    try {
      const existing = JSON.parse(localStorage.getItem('saga-sample-movements') || '[]');
        // Attach idempotency_key locally as well so we can retry later
        const localIdempotency = `local-${Date.now()}-${Math.random().toString(36).substr(2,9)}`;
      existing.unshift({
          idempotency_key: localIdempotency,
        id: `local-${Date.now()}-${Math.random().toString(36).substr(2,9)}`,
        timestamp: event.timestamp || new Date().toISOString(),
        userId: event.userId,
        ...
        fromContainerId: event.fromContainerId,
        fromPosition: event.fromPosition,
        toContainerId: event.toContainerId,
        toPosition: event.toPosition,
        notes: event.notes,
        success: event.success !== false
      });
      localStorage.setItem('saga-sample-movements', JSON.stringify(existing.slice(0, 10000)));
    } catch (storageErr) {
      console.error('Failed to store movement locally:', storageErr);
    }
  }
}

// Fetch recent movements from server (falls back to localStorage)
export async function fetchMovements(limit = 200): Promise<any[]> {
  try {
    const response = await fetchWithTimeout(`${API_BASE_URL}/sample-movements?limit=${limit}`);
    if (!response.ok) throw new Error(`HTTP ${response.status}: ${response.statusText}`);
    const data = await response.json();
    return data.movements || data || [];
  } catch (err) {
    console.warn('Failed to fetch movements from server, falling back to localStorage:', err);
    try {
      const existing = JSON.parse(localStorage.getItem('saga-sample-movements') || '[]');
      return Array.isArray(existing) ? existing : [];
    } catch (storageErr) {
      console.error('Failed to parse local movements:', storageErr);
      return [];
    }
  }
}
